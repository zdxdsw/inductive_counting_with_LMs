...
try:
    ...
    warnings.warn("CUDA extension for structured kernels (Cauchy and Vandermonde multiplication) found.")
except:
    warnings.warn(
...
def dplr(
    ...
    # Initialize B
    if B_random:
        warnings.warn("'B_random' is deprecated in favor of B_init='random' and will be deprecated in a future version.")
    if init in ['legs', 'hippo']:
        warnings.warn(f'Initializing with S4D-LegS and ignoring argument {B_init=}')
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.verbose and len(kwargs) > 0:
            print(f"{type(self)} extra kwargs: {kwargs}")
    ...
    def __init__(
        ...
        **init_args,
        ...
        if measure is not None:
            warnings.warn("Warning: 'measure' option changed to 'init' and will be removed in a future version.")
    ...
    def __init__(
        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        if postact is not None:
            ...
            warnings.warn("Warning: 'postact' option changed to 'final_act' and will be removed in a future version.")
    ...
    def __init__(
        ...
        for _ in range(config.num_hidden_layers):
            self.s4_layers.append(
                S4Block(config.hidden_size, dropout=config.dropout, transposed=True, lr=min(0.001, config.learning_rate))
            )
            self.norms.append(nn.LayerNorm(config.hidden_size))
            self.dropouts.append(nn.Dropout1d(config.dropout))
